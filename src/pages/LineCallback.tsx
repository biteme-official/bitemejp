...
import { requestWelcomeCoupon } from '@/lib/lineWelcomeDiscount';
  ...
  useEffect(() => {
    ...
    handleLineCallback(code, state)
      .then((profile) => {
        ...
        // 이 고객만 쓸 수 있는 쿠폰을 발급받아 저장해둔다. 체크아웃 생성 시
        // 자동으로 붙는다(cartStore). 발급이 실패해도 로그인은 그대로 진행한다.
        if (profile.lineSessionToken) {
          void requestWelcomeCoupon(profile.lineSessionToken);
        }
